...
function extractBargainTextFallback(value) {
  ...
  const offerPrice = firstMatchedPrice(text, [
    /(?:对方|买家)(?:一上来)?(?:只)?\s*(?:出价|出|报价|报|还价|砍到|刀到)?\s*(?:是|为|到)?\s*[¥￥]?\s*([\d.零〇一二两三四五六七八九十百千万点]+)/i,
  ...
  const itemMatch = text.match(/(?:我在卖|我卖|卖的是|这件|这个|一件|一条)?\s*([\u4e00-\u9fa5A-Za-z0-9 ]{2,20}?)(?=\s*[，,]?\s*(?:商品)?原价)/);
  const itemName = itemMatch ? itemMatch[1].trim().replace(/^(?:我在卖|我卖|卖的是|这件|这个|一件|一条)/, "") : "";
  let condition = "";
  ...
  if (req.method === "POST" && req.url === "/api/bargain") {
    try {
      ...
      const evidence = { itemName,listingPrice,offerPrice,originalPrice,floorPrice,category,condition,urgency,
        description,note,comparablePrices,cutAmount,cutPercent,level:level.label,
      ...
      return send(res, 200, {
        ...
        notice:[visionNotice,textNotice,aiNotice].filter(Boolean).join(" "),
