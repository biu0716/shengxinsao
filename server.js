...
function marketResearchPrompt({ itemName,category,condition,originalPrice }) {
  const date = new Date().toISOString().slice(0, 10);
  return `今天是${date}。请联网查询中国大陆公开网页，为二手卖家研究“${itemName}”的价格。
用户补充：商品类型${category || "未说明"}，成色${condition || "未说明"}，用户自述原价${originalPrice ?? "未说明"}元。

查询要求：
1. 优先找品牌官网、主流电商当前新品价格，以及公开可访问的二手平台同款信息。
...
function fallbackBargain({ itemName, listingPrice, offerPrice, originalPrice, floorPrice, category, condition, comparablePrices, cutPercent, level, research }) {
...
function unavailableResearch(status, summary) {
  return { status,confidence:"低",summary,newPrice:null,usedListing:null,confirmedSold:null,
    observations:[],limitations:[],sources:[],searchedAt:Date.now(),cached:false };
}
...
async function researchMarket(input) {
  ...
  try {
    ...
    const result = {
      ...
      usedListing:cleanResearchRange(parsed.usedListing,"二手在售挂牌"),
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && req.url === "/api/bargain") {
    try {
      ...
      let searchNotice = "";
      try {
        research = await researchMarket({ itemName,category,condition,originalPrice });
      } catch (error) {
      ...
      if (LLM_READY) {
        try {
          ...
          reasoning = {
            ...
            missing: userConfirmationArray(ai.missing, fallback.missing),
