...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && req.url === "/api/drill") {
    try {
      ...
      if (phase === "chat") {
        const transcript = (history || []).map(m => (m.role === "assistant" ? "骗子" : m.role === "system" ? "练习系统" : "用户") + "：" + m.content).join("\n");
      ...
      if (phase === "score") {
        const transcript = (history || []).map(m => (m.role === "assistant" ? "骗子" : m.role === "system" ? "练习系统" : "用户") + "：" + m.content).join("\n");
        const out = enforceMockCodeSafety(history, await askJSON(DRILL_SCORE_SYS, `场景：${scenarioDesc}\n\n完整对话：\n${transcript}`));
        return send(res, 200, out);
      }
